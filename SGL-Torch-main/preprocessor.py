...
if __name__ == "__main__":
    # Usage
    data = Preprocessor()
    data.load_data("/home/daniilstrunov/nogcn/SGL-Torch-main/dataset/yelp2018/yelp2018.test", sep=",", columns="UI")
    data.drop_duplicates()
    data.filter_data(user_min=1, item_min=1)
    data.remap_data_id()
    # data.split_data_by_leave_out(valid=1, test=1)
    data.split_data_by_ratio(train=1e-100, valid=0.33, test=1-0.33, by_time=True)
    data.save_data()
